 ...
 */
class TimetableStylesBackend extends Backend {
	...
	public function generateDeleteButton($row, $href, $label, $title, $icon, $attributes) {
		...
		$statistics = $this->Database->prepare("SELECT COUNT(*) AS c FROM tl_timetable WHERE style=?")->execute($row['id'])->row();

		if ($statistics['c'] > 0) {
			$icon = 'delete_.svg';
			$label = sprintf($GLOBALS['TL_LANG']['tl_timetable_styles']['delete_'][0], $row['id']);
			$title = sprintf($GLOBALS['TL_LANG']['tl_timetable_styles']['delete_'][1], $row['id']);
			$tagname = 'a';
			$tagdata = ' href=""';
